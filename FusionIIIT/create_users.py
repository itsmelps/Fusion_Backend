...
from applications.globals.models import DepartmentInfo, ExtraInfo, Designation, HoldsDesignation
...
user = User.objects.create_user(
    ...
    first_name='Test',
    last_name='User',
    email='testuser@iiitdmj.ac.in'
)

# 2. Create or get a department
dept, _ = DepartmentInfo.objects.get_or_create(name='CSE')

# 3. Create the ExtraInfo profile (id is required — use a roll number or employee ID)
...
# Verify
print(User.objects.filter(username='testuser').exists())       # True
print(ExtraInfo.objects.filter(id='2021001').exists())         # True
print(HoldsDesignation.objects.filter(user=user).exists())     # True

from django.contrib.auth.models import User
from applications.globals.models import DepartmentInfo, ExtraInfo, Designation, HoldsDesignation, Faculty, Staff
...
if not User.objects.filter(username='student1').exists() and not ExtraInfo.objects.filter(id='2021002').exists():
    ...
    print("→ Student user already exists (testuser or student1)")
...
if not User.objects.filter(username='faculty1').exists():
    ...
    Faculty.objects.create(id=extra_faculty)
...
if not User.objects.filter(username='staff1').exists():
    ...
    Staff.objects.create(id=extra_staff)
...
# ============ 4. COMPOUNDER USER ============
if not User.objects.filter(username='compounder1').exists():
